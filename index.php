...
include "koneksi.php";
$conn = connect();
// get all data from tb_korpus
$sql_korpus = "SELECT * FROM tb_korpus";
$result_korpus = mysqli_query($conn, $sql_korpus);
$korpus = array();
if (mysqli_num_rows($result_korpus) > 0) {
    while ($row = mysqli_fetch_assoc($result_korpus)) {
        $korpus[] = $row;
    }
}
var_dump($korpus);
...
// change $korpus sentence tobe lower
foreach ($korpus as $key => $value) {
    $korpus[$key]['kalimat'] = strtolower($value['kalimat']);
}
// remove special character
foreach ($korpus as $key => $value) {
    $korpus[$key]['kalimat'] = preg_replace("/[^A-Za-z0-9\  ]/", "", $value['kalimat']);
}
// remove number
foreach ($korpus as $key => $value) {
    $korpus[$key]['kalimat'] = preg_replace('/\d+/u', '', $value['kalimat']);
}
// tokenizing
foreach ($korpus as $key => $value) {
    $korpus[$key]['kalimat'] = explode(' ', $value['kalimat']);
}
// remove empty array
foreach ($korpus as $key => $value) {
    $korpus[$key]['kalimat'] = array_filter($value['kalimat'], function($value) { return !is_null($value) && $value !== ''; });
}
// show tokenizing result as table
echo "<h1> Tokenizing </h1>";
echo "<table border='1'>";
echo "<tr><th> No </th><th> ID </th><th> Kalimat </th><th> Token </th></tr>";
$no=1;
foreach ($korpus as $key => $value) {
    echo "<tr><td>".$no."</td><td>".$value['id']."</td><td>".$value['kalimat']."</td><td>";
    foreach ($value['kalimat'] as $token) {
        echo $token.", ";
    }
    echo "</td></tr>";
    $no++;
